...
%token NULL   EXTENDS     THIS     WHILE   FOR REPEAT 
%token IF     ELSE        RETURN   BREAK   NEW UNTIL
...
%token ','  ';'  '!'  '('  ')'  '['  ']'  '{'  '}'
// New features
%token SELF_INC SELF_DEC '?' ':' SWITCH CASE DEFAULT
...
SwitchBlock       : '{' CaseList DefaultReceiver '}'
                    {
                        $$.stmt = new SwitchBlock($2.slist, $3.stmt, $1.loc);
                    }
                ;

CaseBlock       :   StmtList
                    {
                        $$.stmt = new CaseBlock($1.slist, $1.loc);
                    }
                ;
...
SwitchStmt      :   SWITCH '(' Expr ')' SwitchBlock
                    {
                        $$.stmt = new Tree.Switch($3.expr, (SwitchBlock)$5.stmt, $1.loc);
                    }
                ;
...
DefaultStmt     :   DEFAULT ':' CaseBlock
                    {
                        $$.stmt = new Tree.Default((CaseBlock)$3.stmt, $1.loc);
                    }
                ;
